...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack = ['ROOT']
        self.buffer = [copy_word for copy_word in self.sentence]
        self.dependencies = []
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses)>0:
        lup = len(unfinished_parses)
        batch = unfinished_parses[:batch_size] if lup>=batch_size else unfinished_parses[:]
