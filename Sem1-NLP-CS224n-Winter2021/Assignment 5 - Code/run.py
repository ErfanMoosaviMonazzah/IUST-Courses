...
print(f'RUN will use CUDA: {torch.cuda.is_available()}')
NUM_WORKERS = 1
print(f'RUN will use {NUM_WORKERS} workers')
...
if args.function == 'pretrain':
    ...
    tconf = trainer.TrainerConfig(max_epochs=650, batch_size=128, learning_rate=6e-3, lr_decay=True, warmup_token=512 * 20, final_tokens=200 * len(pretrain_dataset) * block_size, num_workers=NUM_WORKERS)
    MODEL.to(device)
    ...
    tconf = trainer.TrainerConfig(max_epochs=75, batch_size=256, learning_rate=6e-4, lr_decay=True, warmup_tokens=512 * 20, final_tokens=200 * len(pretrain_dataset) * block_size, num_workers=NUM_WORKERS)
