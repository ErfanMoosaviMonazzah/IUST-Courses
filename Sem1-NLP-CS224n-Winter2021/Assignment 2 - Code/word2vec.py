...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    smax = softmax(np.dot(outsideVectors, centerWordVec))
    loss = -np.log(smax[outsideWordIdx])

    smax[outsideWordIdx] -= 1
    gradCenterVec = np.dot(outsideVectors.T, smax)

    gradOutsideVecs = np.dot(smax.reshape((-1, 1)),centerWordVec.reshape((1, -1)))
...
def negSamplingLossAndGradient(
    ...
    first_d = outsideVectors[outsideWordIdx].T.dot(centerWordVec)
    neg_samples = outsideVectors[negSampleWordIndices] 
    
    second_d = -neg_samples.dot(centerWordVec)
    loss = -np.log(sigmoid(first_d)) - np.sum(np.log(sigmoid(second_d)))
    gradCenterVec = -(1 - sigmoid(first_d)) * outsideVectors[outsideWordIdx] + np.sum(((1 - sigmoid(second_d)).reshape((-1,1)) * neg_samples), axis=0)

    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = -(1 - sigmoid(first_d)) * centerWordVec
    for i,neg_idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_idx] += (1 - sigmoid(second_d)[i]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for w in outsideWords:
        glosses = list(word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[w], outsideVectors, dataset))
        loss += glosses[0] 
        gradCenterVecs[word2Ind[currentCenterWord]] += glosses[1]
        gradOutsideVectors += glosses[2]
    ### END YOUR CODE
