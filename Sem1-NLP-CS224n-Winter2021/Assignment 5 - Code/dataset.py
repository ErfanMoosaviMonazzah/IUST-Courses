...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        doc = self.data[idx]
        rmax = (self.block_size * 7 // 8)
        truncate_len = min(len(doc), random.randint(4, rmax))
        truncated_doc = doc[:truncate_len]
        rmin, rmax = (truncate_len // 8), (truncate_len * 3 // 8)
        ml = random.randint(rmin, rmax)
        pl = random.randint(1, truncate_len - ml - 1)

        prefix = truncated_doc[:pl]
        masked_content = truncated_doc[pl:pl + ml]
        suffix = truncated_doc[pl + ml:]

        m_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.PAD_CHAR * (
            self.block_size - truncate_len - 2)

        x = torch.LongTensor([self.stoi[c] for c in m_string[:-1]])
        y = torch.LongTensor([self.stoi[c] for c in m_string[1:]])
        return x, y
