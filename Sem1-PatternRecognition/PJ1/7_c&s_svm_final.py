...
def read(digits, dataset = "training", path = "bin_data"):
    """
    ...
    if kernel == 'linear':
        ...
        def classifier(Y):
            ...
            for i in range(M):
              ...
              # a.sort(reverse = True)
              # a = zip(sorted(list(S[i,:], reverse = True), sorted(range(m), reverse = True)))
              a = sorted(list(zip(list(S[i,:]), range(m))), reverse = True)
              c += [ a[0][1] ]
            return c
...
X, labels = read(digits, "training")
Ntrain = X.size[0]
print(f'Number of Samples: {Ntrain}')
...
print(f'Training Crammer-Singer SVM...')
classifier, utime, iters = mcsvm(X, labels, gamma = gamma)
s = classifier(X)
...
# %%
with open('scrammer_svm_results.pic', 'w') as file:
    dic = {
    'N': Ntrain, 
    'gamma': gamma, 
    'utime': utime,
